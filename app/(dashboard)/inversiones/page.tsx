...
export default function InversionesPage() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/movimientos?desde=${desde}&hasta=${hasta}&tipo=INVERSION`)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-jy-card rounded-xl p-4 border border-jy-amber/20 flex items-center justify-between">
        <span className="text-jy-secondary text-sm">Total invertido en el período</span>
        <span className="text-jy-amber font-display font-semibold text-xl">{formatPesos(total)}</span>
      </div>

      {cargando ? (
        <div className="space-y-2">{[...Array(3)].map((_, i) => <div key={i} className="h-14 bg-jy-card rounded-xl animate-pulse" />)}</div>
      ) : !movimientos.length ? (
        <div className="text-center py-12 text-jy-secondary">
          <p>No hay inversiones registradas en este período.</p>
        </div>
      ) : (
